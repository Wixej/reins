...
- **Голосовое общение**: добавлен режим разговора прямо в окне чата. Приложение слушает пользователя, отправляет распознанный текст модели, стримит ответ и озвучивает его через системный TTS.
- **Защита от самопрослушивания**: во время озвучивания ответа микрофон отключается, чтобы приложение не отправляло обратно фразы, которые произнес TTS.
- **Показ распознанного текста**: во время голосового режима видно, что именно распознала система.
- **Веб-поиск**: добавлена настройка интернет-поиска. Для свежих данных клиент может подмешивать найденные источники в запрос к модели.
- **Специализированные источники**: для валют, погоды, криптовалют и новостей используются отдельные источники, а для общих вопросов остается поиск через DuckDuckGo.
- **Индикатор ответа**: над ответом показывается время до первого текста и маршрут ответа, например быстрый ответ или интернет.
- **Память о пользователе**: в настройках можно указать факты о себе. Эти данные подмешиваются к запросам вместе с текущей датой и временем устройства.
- **Роли модели**: в настройках чата можно выбрать поведение модели, например по умолчанию, кодер, доктор, психолог и другие роли.
- **Настройки модели**: добавлены удобные параметры удержания модели в памяти, отключения мышления, температуры, seed и других опций.
- **Вложения**: добавлена работа с PDF, TXT и Markdown-файлами, а также более наглядное отображение прикрепленных документов в чате.
- **Камера и изображения**: можно прикреплять изображения из галереи или сразу делать фото с камеры.
- **Поиск по чатам**: добавлен поиск по истории сообщений.
- **Темы оформления**: кроме светлой, темной и системной темы добавлена отдельная черная тема с более глубоким фоном и мягкими контейнерами.
- **Android-сборка**: настроена сборка APK для проверки на Android.
