...
class _ServerSettingsState extends State<ServerSettings> {
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            labelText: 'Адрес сервера Ollama',
        ...
        SizedBox(
          ...
          child: Wrap(
            spacing: 8.0,
            runSpacing: 8.0,
            alignment: (Platform.isAndroid || Platform.isIOS) ? WrapAlignment.spaceEvenly : WrapAlignment.spaceBetween,
            children: [
              ElevatedButton(
                onPressed: _isLoading ? null : _handleSearchLocalNetwork,
                child: const Text('Поиск в локальной сети'),
              ),
              ElevatedButton(
  ...
  _handleConnectButton() async {
    ...
    try {
      ...
      _serverAddressErrorText = 'Неверный формат URL. Используй: http(s)://<хост>:<порт>';
      _requestState = OllamaRequestState.error;
    } finally {
      setState(() {});
    }
  ...
  static Future<(OllamaRequestState, Uri)> _establishServerConnection(
    ...
    try {
      final response = await http.get(serverAddress).timeout(const Duration(seconds: 2));
  ...
  String _validateServerAddress(String address) {
    if (address.isEmpty) {
      throw OllamaException('Введите адрес сервера.');
    }
    ...
    if (url.scheme.isEmpty) {
      throw OllamaException(
        'Укажи схему адреса. Например: http://localhost:11434',
      );
    }
    ...
    if (url.host.isEmpty) {
      throw OllamaException(
        'Укажи хост. Например: http://localhost:11434',
      );
    }

    if (url.scheme != 'http' && url.scheme != 'https') {
      throw OllamaException(
        'Неверная схема. Поддерживаются только http и https.',
      );
    }
  ...
  void _handleSearchLocalNetwork() async {
    ...
    try {
      ...
      _serverAddressErrorText = 'Во время поиска произошла ошибка.';
      _requestState = OllamaRequestState.error;
    } finally {
      setState(() {});
    }
  ...
  static Future<(OllamaRequestState, Uri)> _searchLocalNetwork() async {
    ...
    final result = results.firstWhere(
      ...
      orElse: () => throw OllamaException('Сервер Ollama в локальной сети не найден.'),
    );
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          OllamaBottomSheetHeader(title: 'Что такое Ollama?'),
          Divider(),
          Expanded(
            child: ListView(
              children: [
                MarkdownBody(
                  data:
                      "Ollama — это бесплатная платформа, которая позволяет запускать современные большие языковые модели прямо на вашем устройстве. Это дает больше приватности, контроля и гибкости при работе с ИИ. В Ollama также можно настраивать и создавать собственные модели.\n\nЧтобы начать работу, откройте официальный сайт: [ollama.com](https://ollama.com). Там можно посмотреть доступные модели и скачать платформу.",
