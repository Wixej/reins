...
void main() async {
  ...
  test('Test database update chat options', () async {
    ...
    await service.updateChat(
      ...
      newOptions: OllamaChatOptions(
        ...
        temperature: 0.1,
        seed: 1,
        keepAlive: OllamaKeepAliveOption.infinite,
    ...
    expect(updatedChat.options.keepAlive, OllamaKeepAliveOption.infinite);
    expect(updatedChat.options.thinkingEnabled, isFalse);
  });
  ...
  test("Test database get all chats", () async {
    ...
    final retrievedChat = chats.firstWhere((item) => item.id == chat.id);

    expect(retrievedChat.id, isNotEmpty);
    expect(retrievedChat.model, model);
    expect(retrievedChat.title, "New Chat");
    expect(retrievedChat.systemPrompt, isNull);
    expect(retrievedChat.options.toJson(), OllamaChatOptions().toJson());
  });
